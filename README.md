...
3. **Redis**：作为消息发送记录的缓存存储，以提高消息记录的查询速度。
...
7. **RabbitMQ延迟队列**：通过引入延迟队列，处理超时消息任务，提高消息的可靠性。
8. **链路追踪**：使用Redis实现消息的链路追踪，对消息的各个阶段进行实时监控，日志记录，掌控消息的生命周期。
9. **Mysql**：作为存储消息发送模板信息和第三方账号配置信息的数据库。
10. **ECharts可视化**：通过使用ECharts，对消息模板下发用户数、今日消息送达率、每天各时间段发送情况以及消息模板用户等数据进行可视化展示，方便进行消息模板的数据分析。
11. **Sentinel限流**：使用Sentinel来对消息发送接口进行限流，保障系统的稳定性。

## 项目特性
多渠道消息触达平台具有以下特性：

1. 统一提供多个消息服务渠道：与多个第三方消息服务API进行对接，包括邮件、短信、钉钉、APP通知栏、微信公众号和飞书机器人。
2. 
